          ...
          <ul className='choices-box'>
            {choices.map((answer, index) => (
              <li
                ...
                <button className={`${selected ? 'cursor-default' : ''} ${loading ? 'btnQuizNotActive' : 'btnQuiz'} font-poppins ${selectedAnswerIndex === index && selected ? 'fontSelected' : ''}` }  disabled={loading} onClick={() => onAnswerSelected(index)}>{answer}</button>
              </li>
            ))} 
              ...
              <ul className='votes-box' style={{backgroundImage: `url(${granite.src})`, height: !selected ? '0px' : '', padding: !selected ? '0px' : '', border: !selected ? 'none' : '', marginBottom: !selected ? '0px' : '' }}>
                {votes.map((vote, index) => (
                  <div className='totalQuiz' key={index}>
                    ...
                    <div className='resultQuiz__votes'>{`${percentage[index] ? percentage[index] : 0} / ${percentage[index] ? currentVotes + 1 : currentVotes}`}</div>
                  </div>
                ))}
              </ul>
            </div>
